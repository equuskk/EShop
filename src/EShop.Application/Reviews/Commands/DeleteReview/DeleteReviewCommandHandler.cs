...
namespace EShop.Application.Reviews.Commands.DeleteReview
{
    public class DeleteReviewCommandHandler : IRequestHandler<DeleteReviewCommand, Unit>
    {
        ...
        {
            _logger.Debug("Удаление отзыва {0}", request.ReviewId);
            var review = await _db.Reviews.FirstOrDefaultAsync(x => x.Id == request.ReviewId,
                                                               cancellationToken);

            if(review is null)
            {
                _logger.Debug("Отзыв {0} не найден", request.ReviewId);
                throw new NotFoundException(nameof(Review), request.ReviewId);
            }

            if(request.ShopUserId != review.UserId)
            {
                _logger.Debug("Несоответствие UserId при удалении отзыва ({0} != {1})",
                              request.ShopUserId, review.UserId);
            ...
            _logger.Debug("Отзыв {0} удалён", request.ReviewId);
